class Promiscuous::Subscriber::Worker::Distributor
  ...
  def start
    ...
    Promiscuous::Config.subscriber_topics.each do |topic|
      ...
      puts "[distributor] Started #{num_threads} thread#{'s' if num_threads>1} topic:#{topic}"
    end
    puts "[distributor] Started #{@distributor_threads.count} total threads"
  end

  def stop
    return if @distributor_threads.blank?
    puts "[distributor] Stopping #{@distributor_threads.count  } threads"
  ...
  class DistributorThread
    def initialize(topic)
      ...
      puts "[distributor] Subscribing to topic:#{topic} [#{@thread.object_id}]"
    end
    ...
    def main_loop(topic)
      ...
      while not @stop do
        begin
          puts "fetching and processing messages for #{@consumer.topic}"
          fetch_and_process_messages(&method(:on_message))
        rescue Poseidon::Connection::ConnectionFailedError
          puts "[kafka] Reconnecting... [#{@thread.object_id}]"
    ...
    def stop
      puts "[distributor] stopping status:#{@thread.status} [#{@thread.object_id}]"
      ...
      @stop = true
      @thread.join
      puts "[distributor] stopped [#{@thread.object_id}]"
    end
